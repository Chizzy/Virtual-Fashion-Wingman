...
UserHelp.create({
    ...
    const post1 = Post.create({
        ...
        creator: johnny._id,
        outfit: 'La croix vegan intelligentsia fixie lumbersexual shaman. Fingerstache authentic kale chips plaid.',
        feedback: rami.feedbacks[2]
    }).then((post) => {
    ...
    const post2 = Post.create({
        ...
        creator: johnny._id,
        outfit: "Flannel crucifix fixie master cleanse, biodiesel activated charcoal cloud bread bushwick. Whatever you probably haven't heard of them thundercats gastropub chia.",
        feedback: janelle.feedbacks[1]
    }).then((post) => {
        johnny.posts.push(post)
    })
...
UserHelp.create({
    ...
    const post1 = Post.create({
        ...
        creator: nicki._id,
        outfit: "Pok pok small batch street art you probably haven't heard of them. Chicharrones mustache fashion axe semiotics tumeric plaid PBR&B kinfolk bushwick subway tile microdosing schlitz.",
        feedback: janelle.feedbacks[2]
    }).then((post) => {
    ...
    const post2 = Post.create({
        ...
        creator: nicki._id,
        outfit: "Letterpress vaporware tumblr meh gastropub chillwave echo park, shoreditch brooklyn direct trade shabby chic. Leggings kombucha tofu, polaroid church-key fashion axe try-hard unicorn swag meh small batch tilde copper mug wayfarers.",
        feedback: rami.feedbacks[1]
    }).then((post) => {
        nicki.posts.push(post)
    })
    return Promise.all([post1, post2]).then(() => {
